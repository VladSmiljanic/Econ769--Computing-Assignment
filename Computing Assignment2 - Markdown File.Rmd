...
stargazer(pooled,within,random,between,fd,no.space=TRUE,omit.stat = c("f"),column.labels = c("Pooled","Within","Random","Between","First-Diff."))
```
...
boot_se<-function(data,R,model,effect){
  unique_ind<-unique(data$id)
  samp_ind<-list()
  for (j in 1:R){
...
col.n<-c("Coefficients","Pooled","Within","Random Effect","Between","First Difference")
tab<-data.frame(Model=c("lnwg","kids","disab","ageh","agesq"))

tab$pooled<-apply(data.frame(boot_se(mydata,20,"pooling",effect=NULL))[2:6,], 1, sd)
tab$within<-apply(data.frame(boot_se(mydata,20,"within",effect="individual"))[1:5,], 1, sd)
tab$random<-apply(data.frame(boot_se(mydata,20,"random",effect="individual"))[2:6,], 1, sd)
tab$between<-apply(data.frame(boot_se(mydata,20,"between",effect=NULL))[2:6,], 1, sd)
tab$first.difference<-apply(data.frame(boot_se(mydata,20,"fd",effect=NULL))[2:6,], 1, sd)
...
\begin{align*}
E[\epsilon_{it}-\bar \epsilon_i |x_{i1}-\bar x_i,\dots,x_{Nt}]=0
\end{align*}
...
The assumption is made that both $\alpha_i$ and $\epsilon_{it}$ are iid, important for when they are placed in the error term, $u_{it}=\alpha_i+\epsilon_{it}$. 'Random effects' doesn't require more then strict exogeneity and linear independence of transformed data for consistency. The assumptions of homoskedasticity are not important for consistency.
...
random<-plm(lnhr~lnwg+kids+disab+ageh+agesq, data = mydata, model = "random",index=c("id"),effect="individual")
within <- plm(lnhr~lnwg+kids+disab+ageh+agesq, data = mydata, model = "within",index=c("id"),effect="individual")
...
We will take the exponetial of the 'lnhr' variable and divide it by 365. This will produce the number of hours worked per day to the nearest hour:
